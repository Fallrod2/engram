...
| `ENGRAM_DEMO_EMAIL`, `ENGRAM_DEMO_PASSWORD`                                    | Identifiants du compte démo, **côté serveur uniquement** (aucun jumeau `VITE_`). Avec `SUPABASE_URL` + `SUPABASE_ANON_KEY`, ils ouvrent `POST /api/demo/session` et le CTA « Essayer la démo » de la landing ; absents → 503 propre et CTA masqué. |
