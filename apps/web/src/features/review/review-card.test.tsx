...
/**
 * Reduced motion (29/07/2026) — the reveal loses its MOVEMENT and nothing else.
 *
 * `reduce` is the already-arbitrated motion preference: the system's
 * `prefers-reduced-motion`, unless the user has overridden it in Settings. A
 * test cannot say whether an animation is nice; it can say that turning it off
 * changes nothing a screen reader — or a reader — can observe.
 */
describe('<ReviewCard> — reduced motion', () => {
  const PROPS = { qcm: null, selectedChoice: null, onSelect: () => {} } as const

  it('mounts the same content, moving or not', () => {
    const props = { ...PROPS, front: 'Ma question', back: 'La réponse', revealed: true } as const
    const moving = render(<ReviewCard {...props} reduce={false} />)
    const movingHtml = contents(moving.container.querySelector('article') as HTMLElement)
    cleanup()
    const still = render(<ReviewCard {...props} reduce />)
    const stillHtml = contents(still.container.querySelector('article') as HTMLElement)
    // Same nodes, same order, same text — only what the eye sees moving differs.
    expect(stillHtml).toBe(movingHtml)
    expect(screen.getByText('Ma question')).toBeTruthy()
    expect(screen.getByText('La réponse')).toBeTruthy()
  })

  it('keeps the elevation — it is a state, not a gesture', () => {
    // The shadow says "this card is revealed". Dropping it under reduced motion
    // would take away information, not movement; the CSS block in `styles.css`
    // already shortens its transition to nothing.
    const { container } = render(<ReviewCard {...PROPS} front="Q" back="A" revealed reduce />)
...
describe('<ReviewCard> — the reveal', () => {
  const PROPS = { qcm: null, selectedChoice: null, onSelect: () => {} } as const
  ...
  it('deepens the card’s resting elevation once revealed', () => {
    ...
    const asking = render(
      <ReviewCard {...PROPS} front="Q" back="A" revealed={false} reduce={false} />,
    )
    ...
    cleanup()
    const revealed = render(<ReviewCard {...PROPS} front="Q" back="A" revealed reduce={false} />)
  ...
  it('keeps the question on screen once the answer lands', () => {
    // The flip that used to REPLACE the question is gone (Alex, 29/07/2026):
    // the answer is read against the question, and there is no mode left in
    // which the question leaves the DOM.
    const { container } = render(
      <ReviewCard {...PROPS} front="Ma question" back="La réponse" revealed reduce={false} />,
    )
    expect(container.querySelector('[data-slot="question"]')).toBeTruthy()
    expect(container.querySelector('hr')).toBeTruthy()
    expect(screen.getAllByText('Ma question')).toHaveLength(1)
