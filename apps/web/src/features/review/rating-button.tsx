...
/**
 * Projected-interval text color per rating token. Exported because the Next
 * button (rating-bar) prints the same projection for the recorded grade and has
 * to color it identically. Literal class names, same reason as `TOKENS`.
 */
export const INTERVAL_TOKEN: Record<RatingMeta['token'], string> = {
  danger: 'text-danger',
...
export function RatingButton({
  ...
  // e2e/support/selectors.ts) addresses this button with `/^Bien/`.
    ...
    >
      ...
      <span
        className={cn(
          ...
          interval ? INTERVAL_TOKEN[meta.token] : 'text-text-faint',
        )}
      >
        {interval ?? '·'}
      </span>
