...
describe('studyPlan — bucketing & window', () => {
  it('buckets_dues_by_local_day', async () => {
    ...
    const res = await studyPlan(db, U, { from: key(0), to: key(5), now: NOW })
  ...
  it('bucketing_is_local_not_utc', async () => {
    ...
    const res = await studyPlan(db, U, { from: key(0), to: key(2), now: NOW })
    expect(findDay(res, key(0)).dueCount).toBe(1)
    expect(findDay(res, key(1)).dueCount).toBe(1)
  })
  ...
  it('overdue_aggregated_onto_today', async () => {
    ...
    const res = await studyPlan(db, U, { from: key(-7), to: key(5), now: NOW })
  ...
  it('overdue_split_by_subject', async () => {
    ...
    const res = await studyPlan(db, U, { from: key(-7), to: key(2), now: NOW })
  ...
  it('overdue_excluded_when_today_before_window', async () => {
    ...
    const res = await studyPlan(db, U, { from: key(2), to: key(5), now: NOW })
    const total = res.days.reduce((sum, day) => sum + day.dueCount, 0)
    expect(total).toBe(0)
  })
  ...
  it('archived_subject_excluded', async () => {
    ...
    const res = await studyPlan(db, U, { from: key(-2), to: key(2), now: NOW })
    const total = res.days.reduce((sum, day) => sum + day.dueCount, 0)
    expect(total).toBe(0)
  })
  ...
  it('bySubject_splits_by_subject', async () => {
    ...
    const res = await studyPlan(db, U, { from: key(0), to: key(3), now: NOW })
  ...
  it('dense_days_zero_filled', async () => {
    const res = await studyPlan(db, U, { from: key(0), to: key(4), now: NOW })
  ...
  it('subject_filter_scopes_dues', async () => {
    ...
    await seedCard(db, (await seedDeck(db, s1.id)).id, { due: due(1) })
    await seedCard(db, (await seedDeck(db, s2.id)).id, { due: due(1) })
    const res = await studyPlan(db, U, { from: key(0), to: key(3), now: NOW, subjectId: s1.id })
    expect(sub(res, key(1), s1.id)!.dueCount).toBe(1)
...
describe('studyPlan — exams & boost', () => {
  ...
  it('boost_applied_in_ramp_only', async () => {
    ...
    const res = await studyPlan(db, U, { from: key(0), to: key(14), now: NOW })
  ...
  it('no_boost_on_past_ramp_days', async () => {
    ...
    const res = await studyPlan(db, U, { from: key(-5), to: key(5), now: NOW })
  ...
  it('boost_respects_subject_filter', async () => {
    ...
    for (let i = 0; i < 7; i++)
      await seedCard(db, (await seedDeck(db, s2.id)).id, { due: due(100) })
    await seedExam(db, [s1.id, s2.id], { date: due(10, 0) })
    const res = await studyPlan(db, U, { from: key(0), to: key(14), now: NOW, subjectId: s1.id })
    expect(sub(res, key(5), s1.id)!.examBoost).toBe(1)
...
describe('studyPlan — guards & invariants', () => {
  it('from_after_to_400', async () => {
    await expect(studyPlan(db, U, { from: key(5), to: key(0), now: NOW })).rejects.toThrow(
      ValidationError,
    )
  })
  ...
  it('window_too_large_400', async () => {
    await expect(
      studyPlan(db, U, { from: '2026-01-01', to: '2027-12-31', now: NOW }),
  ...
  it('total_equals_due_plus_boost', async () => {
    ...
    const res = await studyPlan(db, U, { from: key(0), to: key(6), now: NOW })
...
describe('studyToday', () => {
  it('today_total_matches_review_counts', async () => {
    ...
    const res = await studyToday(db, U, NOW)
    expect(res.total).toBe((await dueCounts(db, U, NOW)).total)
  })
  ...
  it('today_vs_study_plan_cross_consistency_strict', async () => {
    ...
    const today = await studyToday(db, U, NOW)
    const plan = await studyPlan(db, U, { from: key(0), to: key(1), now: NOW })
