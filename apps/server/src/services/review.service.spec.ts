...
describe('reviewCard (transactional)', () => {
  it('review_persists_card_and_log', async () => {
    ...
    const res = await reviewCard(db, id, { grade: 3, durationMs: 4200 })
  ...
  it('review_null_duration_when_absent', async () => {
    const id = await newCard()
    await reviewCard(db, id, { grade: 3 })
  ...
  it('review_nonexistent_card_throws_notfound', async () => {
    await expect(reviewCard(db, 'missing', { grade: 3 })).rejects.toThrow(NotFoundError)
  ...
  it('review_honors_reviewedAt', async () => {
    ...
    const res = await reviewCard(db, id, { grade: 3, reviewedAt: when })
  ...
  it('review_roundtrip_matches_shared', async () => {
    const id = await newCard()
    const res = await reviewCard(db, id, { grade: 3, durationMs: 100 })
  ...
  it('review_second_review_uses_persisted_state', async () => {
    const id = await newCard()
    const first = await reviewCard(db, id, {
      grade: 3,
      reviewedAt: new Date(Date.now() - 3_600_000),
    })
    const second = await reviewCard(db, id, { grade: 3, reviewedAt: new Date() })
  ...
  it('review_rejects_future_reviewedAt', async () => {
    ...
    await expect(reviewCard(db, id, { grade: 3, reviewedAt: future })).rejects.toThrow(
      ValidationError,
    )
    expect(await db.select().from(reviewLog)).toHaveLength(0)
  })
  ...
  it('review_rejects_reviewedAt_before_lastReview', async () => {
    const id = await newCard()
    await reviewCard(db, id, { grade: 3, reviewedAt: new Date(Date.now() - 1_000) })
  ...
  it('review_uses_injected_scheduler', async () => {
    ...
    const res = await reviewCard(db, id, { grade: 3, reviewedAt: when }, sched)
