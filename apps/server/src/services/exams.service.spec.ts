...
describe('createExam', () => {
  ...
  it('create_exam_normalizes_date_to_local_midnight', async () => {
    ...
    const dto = await createExam(db, { title: 'E', date: local.toISOString(), subjectIds: [s.id] })
...
describe('listExams', () => {
  it('list_exams_sorted_by_date_asc', async () => {
    const s = await seedSubject(db)
    await createExam(db, { title: 'C', date: '2026-08-10T00:00:00.000Z', subjectIds: [s.id] })
    await createExam(db, { title: 'A', date: '2026-08-01T00:00:00.000Z', subjectIds: [s.id] })
    await createExam(db, { title: 'B', date: '2026-08-05T00:00:00.000Z', subjectIds: [s.id] })
    expect((await listExams(db, {})).map((e) => e.title)).toEqual(['A', 'B', 'C'])
  })
  ...
  it('list_exams_filter_by_subject', async () => {
    ...
    const a = await createExam(db, {
      title: 'A',
      date: '2026-08-01T00:00:00.000Z',
      subjectIds: [s1.id, s2.id],
    })
    const b = await createExam(db, {
      title: 'B',
      date: '2026-08-02T00:00:00.000Z',
      subjectIds: [s2.id],
    })
    await createExam(db, { title: 'C', date: '2026-08-03T00:00:00.000Z', subjectIds: [s3.id] })
    const ids = (await listExams(db, { subjectId: s2.id })).map((e) => e.id).sort()
  ...
  it('list_exams_aggregates_subject_ids_no_n_plus_1', async () => {
    ...
    await createExam(db, { title: 'B', date: '2026-08-02T00:00:00.000Z', subjectIds: [s1.id] })
    const list = await listExams(db, {})
...
describe('updateExam', () => {
  it('update_exam_replaces_subject_ids', async () => {
    ...
    const s2 = await seedSubject(db)
    const s3 = await seedSubject(db)
    const e = await createExam(db, {
      title: 'E',
      date: '2026-08-01T00:00:00.000Z',
      subjectIds: [s1.id, s2.id],
    })
    const upd = await updateExam(db, e.id, { subjectIds: [s2.id, s3.id] })
  ...
  it('update_exam_title_only_keeps_subjects', async () => {
    ...
    const upd = await updateExam(db, e.id, { title: 'Renamed' })
  ...
  it('update_exam_subjects_only_bumps_updated_at', async () => {
    ...
    await updateExam(db, e.id, { subjectIds: [s2.id] })
    const [after] = await db.select().from(exam).where(eq(exam.id, e.id))
  ...
  it('update_exam_empty_patch_is_noop', async () => {
    ...
    const [before] = await db.select().from(exam).where(eq(exam.id, e.id))
    const upd = await updateExam(db, e.id, {})
  ...
  it('update_exam_date_renormalized', async () => {
    ...
    await updateExam(db, e.id, { date: new Date(2026, 8, 15, 9, 0).toISOString() })
  ...
  it('update_exam_missing_404', async () => {
    await expect(updateExam(db, 'nope', { title: 'x' })).rejects.toThrow(NotFoundError)
  })
  ...
  it('update_exam_missing_subject_404_does_not_mutate', async () => {
    ...
    await expect(updateExam(db, e.id, { subjectIds: ['nope'] })).rejects.toThrow(NotFoundError)
...
describe('deleteExam', () => {
  it('delete_exam_cascades_junction', async () => {
    ...
    await deleteExam(db, e.id)
    expect(await junctionIds(e.id)).toEqual([])
    await expect(getExam(db, e.id)).rejects.toThrow(NotFoundError)
  })

  it('delete_exam_missing_404', async () => {
    await expect(deleteExam(db, 'nope')).rejects.toThrow(NotFoundError)
  })
})
