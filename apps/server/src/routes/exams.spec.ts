...
describe('exams routes', () => {
  ...
  it('GET /api/exams sorted by date', async () => {
    const s = seedSubject(db)
    seedExam(db, [s.id], { title: 'Later', date: new Date(2026, 7, 10) })
    seedExam(db, [s.id], { title: 'Sooner', date: new Date(2026, 7, 1) })
  ...
  it('PATCH {} → 200 returns the exam unchanged', async () => {
    const s = seedSubject(db)
    const e = seedExam(db, [s.id], { title: 'Keep' })
  ...
  it('DELETE → 204 and junction rows gone', async () => {
    const s = seedSubject(db)
    const e = seedExam(db, [s.id])
    expect((await app.request(`/api/exams/${e.id}`, { method: 'DELETE' })).status).toBe(204)
    const rows = db.select().from(examSubject).where(eq(examSubject.examId, e.id)).all()
