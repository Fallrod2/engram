...
2. ~~**L'app est sur le web public**, protégée uniquement par Vercel Authentication (barrière navigateur/SSO). Ça suffit pour Alex derrière son écran, mais **aucun appelant machine** (cron clawdeck, daemon Obsidian sur le Mac mini) ne peut franchir cette barrière proprement. D'où le besoin d'un **token applicatif** dès qu'on veut intégrer quoi que ce soit.~~
   **→ Périmé (27/07/2026).** La protection n'est pas Vercel Authentication : sur Hobby, la portée qui couvrirait le domaine de prod est indisponible (cf. `docs/deploy-vercel.md` § Protection). L'app est protégée par une **auth applicative Supabase** — gate JWT sur `/api/*` (`apps/server/src/http/auth.ts`) + inscriptions fermées. Le besoin d'un chemin pour les **appelants machine** reste ouvert : il n'existe pas de token applicatif dédié aujourd'hui, un daemon devrait porter un JWT Supabase.
...
> **Mise à jour 27/07/2026 — QCM interactif en cours.** Le point « `kind: 'quiz'` non interactif » est le chantier ouvert. Approche retenue par Alex : **parsing du Markdown au rendu** (recto → question + options `A)`/`B)`/`C)`, verso → lettre de la bonne réponse), avec repli sur le rendu Markdown actuel si la détection échoue. **Aucune migration, aucun changement du générateur IA** — les cartes QCM déjà en base deviennent interactives rétroactivement. La sélection d'une option **révèle** (bonne réponse en vert, choix erroné en rouge) mais **ne note pas** : l'utilisateur garde la barre 1-4, donc la sémantique FSRS est inchangée. Les deux autres angles morts (poids FSRS, régulation des cartes neuves) restent entiers.
...
- ~~**(b) Engage-t-on vraiment le multi-utilisateur ?** L'auth Supabase « vraie » entraîne une dette schéma lourde (`user_id` sur les 8 tables + RLS). Elle ne sert qu'au partage public v2 et au multi-compte. **Recommandation** : ne PAS coder l'auth tant que le multi-user n'est pas engagé ; un simple token en env suffit en mono-user, et le partage de decks se fait très bien par fichier (v1) sans auth.~~
  **→ TRANCHÉE et livrée (27/07/2026) : le multi-utilisateur est engagé.** La recommandation ci-dessus a été dépassée par les faits. Sont en place : gate JWT Supabase sur `/api/*`, scoping `user_id` sur toutes les tables (migration `0004`, backfill compris), BYOK par utilisateur (`0006`), OAuth Codex (`0007`), gate admin (`0008`), groupes RBAC (`0009`), CRUD de comptes via l'API GoTrue Admin, et un compte démo. Toute route lit son identité par `requireUserId(c)`. La dette schéma redoutée a donc été payée ; ce qui reste devant, c'est le **freemium** (clé plateforme pour les payants, BYOK pour les gratuits), séquencé en §2bis.
